...
public class ProgressCardHandler {
	...
	public void handle(Player sender, ProgressCardType card) {
		...
		case COMMERCIAL_HARBOR:
			// give each opponent a resource of your choice in exchange for a
		...
		case RESOURCE_MONOPOLY:
			// name a resource; all other players must give you 2 of that if
			// they have
			break;
		case TRADE_MONOPOLY:
			// name a commodity; all players must give you 1 if that if they
			// have it
			break;
		case BISHOP:
			// move the robber, but get a random card from EACH player on the
			// robber's new hex
			break;
		case CONSTITUTION:
			// one VP for player
			break;
		case DESERTER:
			// choose an opponent who must remove a knight; place a knight of
			// equal strength
			break;
		case DIPLOMAT:
			// remove any open road; if it's your own, you may place it
			// somewhere else
			break;
		case INTRIGUE:
			// choose an opponent's knight to displace for free, but it must be
			// on your road network
			break;
		case SABOTEUR:
		...
		case WARLORD:
			...
			break;
		case WEDDING:
			// All players with more VPs than you must give you 2 resources of
			// their choice
			break;
		case CRANE:
			// Your next city improvement you build this turn costs one less
		...
		case ENGINEER:
			...
			break;
		case INVENTOR:
			// Swap the numbers of two hexagons on the board. They must not be
			// 2, 6, 8, 12
			break;
		case IRRIGATION:
			// Draw two grain for each field tile you have at least one village
			// on
			for (int y = 0; y < 7; y++) {
				for (int x = 0; x < 7; x++) {
		...
		case MEDICINE:
			...
			break;
		case MINING:
			// Draw two ore for each mountain tile you have at least one village
			// on
			for (int y = 0; y < 7; y++) {
				for (int x = 0; x < 7; x++) {
					Hexagon h = game.getGameBoardManager().getBoard().getHexagonAt(x, y);
