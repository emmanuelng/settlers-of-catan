...
public class GetListOfPlayersCommand implements ClientToServerCommand {
	...
	public void execute(Session sender, Server server) {
		...
		try {
			System.out.println(server.getGameManager().getGameById(gameID).getPlayersManager().getParticipantsUsernames());
			sender.sendCommand(new PlayerListResponseCommand(server.getGameManager().getGameById(gameID).getPlayersManager().getParticipantsUsernames()));
		} catch (IOException e) {
			// Ignore
		}
	}
